...
from nandemo_oshirase.lambda_function import LineMessage, split_into_batches
...
class TestSplitIntoBatches:
    ...
    def test_split_into_batches_under_limit(self):
        messages: list[LineMessage] = [{"type": "text", "text": f"msg{i}"} for i in range(3)]
        result = split_into_batches(messages)
        assert result == [messages]

    def test_split_into_batches_exactly_five(self):
        messages: list[LineMessage] = [{"type": "text", "text": f"msg{i}"} for i in range(5)]
        result = split_into_batches(messages)
        assert result == [messages]

    def test_split_into_batches_six_messages(self):
        messages: list[LineMessage] = [{"type": "text", "text": f"msg{i}"} for i in range(6)]
        result = split_into_batches(messages)
        assert len(result) == 2
    ...
    def test_split_into_batches_ten_messages(self):
        messages: list[LineMessage] = [{"type": "text", "text": f"msg{i}"} for i in range(10)]
        result = split_into_batches(messages)
        assert len(result) == 2
        assert len(result[0]) == 5
    ...
    def test_split_into_batches_eleven_messages(self):
        messages: list[LineMessage] = [{"type": "text", "text": f"msg{i}"} for i in range(11)]
        result = split_into_batches(messages)
        assert len(result) == 3
        assert len(result[0]) == 5
        assert len(result[1]) == 5
    ...
    def test_split_into_batches_custom_size(self):
        messages: list[LineMessage] = [{"type": "text", "text": f"msg{i}"} for i in range(7)]
